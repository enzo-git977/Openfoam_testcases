...
Cantera_path = 'Cantera_solution_Lean/data_new_mech/cantera_profile.csv'
time = "0.02"
OF_path = os.path.join(current_dir, "Openfoam_solution_Lean", "postProcessing", "axialLine", time, "line1.xy")
output_dir = 'figures/'
...
idx_peak_foam = np.argmin(np.abs(T_foam - T_target))# OpenFOAM
...
for sp in species_all:
    ...
    error = (np.abs(phi_of- phi_cant)/(phi_cant+1e-12)) # +1e-12 : avoir 0 division
    dict_error[sp] = error
    
print("Plotting barchart of integral error")
# plot using barchart
plt.figure(figsize=(12,5))
plt.bar(dict_error.keys(),dict_error.values(),color='black')
...
plt.plot(x_foam_shifted[::5], T_foam[::5], label='OpenFOAM', color='navy', marker='o', linestyle='none')
