...
grid=np.linspace(0,width,361)

gas = ct.Solution('mech/New_h_mech.yaml')
#gas = ct.Solution('mech/h2o2.yaml')
...
try:
    ...
    #f.set_refine_criteria(ratio=3.0, slope=0.03, curve=0.03, prune=0.001)
...
for i, sp in enumerate(species_to_plot):
    axs[i].plot(f.grid * 1000, f.Y[gas.species_index(sp), :], label=sp, color= 'black')
