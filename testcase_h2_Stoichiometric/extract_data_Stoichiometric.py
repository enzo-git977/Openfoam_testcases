...
Fields_DIR = os.path.join(CURRENT_DIR, "Openfoam_solution_Stoich","postProcessing","probes","0") # directory of the fields file
...
for filename in field_files:
    ...
    try:
        ...
        df = pd.read_csv(file_path, sep=r'\s+', header=None, skiprows=2, names=['time', field_name])
        field_dfs.append(df)
...
if field_dfs:
    ...
    output_file = "data_Foam_Stoichiometric.csv"
